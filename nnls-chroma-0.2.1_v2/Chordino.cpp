...
Chordino::getRemainingFeatures()
{
    ...
    **/
    ...
    for (FeatureList::iterator i = m_logSpectrum.begin(); i != m_logSpectrum.end(); ++i) {
       ...
       // cerr << "CONVERSION - "  <<  intShift << " " << floatShift << endl;
    ...
    **/
    ...
    FeatureList chromaList;
     ...
     vector<pair<string,int> > myVector1;
        myVector1.push_back(make_pair("A",0));
        myVector1.push_back(make_pair("Bb",0));
        myVector1.push_back(make_pair("B",0));
        myVector1.push_back(make_pair("C",0));
        myVector1.push_back(make_pair("C#",0));
        myVector1.push_back(make_pair("D",0));
        myVector1.push_back(make_pair("Eb",0));
        myVector1.push_back(make_pair("E",0));
        myVector1.push_back(make_pair("F",0));
        myVector1.push_back(make_pair("F#",0));
        myVector1.push_back(make_pair("G",0));
        myVector1.push_back(make_pair("Ab",0));
        ...
        // scale contains 12 major and then 12 minor scales

        vector<string> scale;
        for(int ii=3;ii<15;ii++)
            scale.push_back(m_chordnames[ii]);

        for(int ii=3;ii<15;ii++)
        {   string t = m_chordnames[ii];
            t.append("m");
            scale.push_back(t);
        }
        

        vector<vector<string> > notes_;
        for(int ii=0;ii<12;ii++)
        {
            vector<string> temporary;
            temporary.push_back(scale[ii%12]);
            temporary.push_back(scale[(ii+2)%12]);
            temporary.push_back(scale[(ii+4)%12]);
            temporary.push_back(scale[(ii+5)%12]);
            temporary.push_back(scale[(ii+7)%12]);
            temporary.push_back(scale[(ii+9)%12]);
            temporary.push_back(scale[(ii+11)%12]);
            notes_.push_back(temporary);
        }
        ...
        for(int ii=0;ii<12;ii++)
        {
            ...
            temporary.push_back(scale[ii%12]);
            temporary.push_back(scale[(ii+2)%12]);
            temporary.push_back(scale[(ii+3)%12]);
            temporary.push_back(scale[(ii+5)%12]);
            temporary.push_back(scale[(ii+7)%12]);
            temporary.push_back(scale[(ii+8)%12]);
            temporary.push_back(scale[(ii+10)%12]);
            notes_.push_back(temporary);
               
        }

        vector<string> maj_min;
        maj_min.push_back("");
        maj_min.push_back("m");
        maj_min.push_back("m");
        maj_min.push_back("");
        maj_min.push_back("");
        maj_min.push_back("m");
        maj_min.push_back("dim");
        
        
        // for(int jj =0;jj<notes_.size();jj++)
        // {for(int ii=0;ii<7;ii++)
        //     cerr << notes_[jj][ii] << " ";
        // cerr << endl;
    
    vector<vector<float> > chroma_features; 
...
chroma_features.push_back(chroma);
        ...
        for (int iChord = 0; iChord < nChord; iChord++) {
            ...
            {
                myVector1[ii-12].second += chroma[ii];
            }
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////
    sort(myVector1.begin(),myVector1.end(),cmp);
    //         for(int ii=0;ii<7;ii++)
    //             map1[myVector1[ii].first] += 1;
    // vector<pair<string,int> > v(map1.size());
    // copy(map1.begin(), map1.end(), v.begin());
    // sort(v.begin(),v.end(),cmp);
    for(int ii=0;ii<myVector1.size() ;ii++)
        cerr << myVector1[ii].first << " " << myVector1[ii].second << endl;


    vector<string> found_notes;
    found_notes.push_back(myVector1[11].first);
    found_notes.push_back(myVector1[10].first);
    found_notes.push_back(myVector1[9].first);
    found_notes.push_back(myVector1[8].first);
    found_notes.push_back(myVector1[7].first);
    found_notes.push_back(myVector1[6].first);
    found_notes.push_back(myVector1[5].first);
    vector<string> intersect(7);
    int maxsize =0;
    int index_scale=0;
    sort(found_notes.begin(),found_notes.end());
    for(int ii=0;ii<notes_.size();ii++)
    {
        vector<string> temp_notes = notes_[ii];
        sort(temp_notes.begin(),temp_notes.end());
        vector<string>::iterator it;
        it=set_intersection (found_notes.begin(), found_notes.end(), temp_notes.begin(), temp_notes.end(), intersect.begin());
                                              
        intersect.resize(it-intersect.begin()); 
        if(intersect.size() > maxsize) {maxsize=intersect.size(); index_scale=ii; }
    }

    cerr << "SCALE "<<scale[index_scale]<<  endl;
    for(int ii=0;ii<7;ii++)
            cerr<< notes_[9][ii] << " ";
        cerr << endl;
    ...
    for (int ii = 0; ii < 7; ++ii)
    {
        string t = notes_[index_scale][ii];
        t.append(maj_min[ii]);

        chords.push_back(t);
    }
    for(int ii=0;ii<chords.size();ii++)
        cerr << chords[ii] << " " ;
    cerr << endl;
	...
	if (m_useHMM == 1.0) {
        ...
        vector<int> chordpath = ViterbiPath(init, trans, chordogram, delta,m_chordnames,chords);
        ...
        for (int iFrame = 1; iFrame < (int)chordpath.size(); ++iFrame) {
            ...
            if (chordpath[iFrame] != oldchord ) {
                ...
                cerr << "CHORD -------------------- " <<  m_chordnames[chordpath[iFrame]] << "  "; 
        ...
        for (vector<Vamp::RealTime>::iterator it = timestamps.begin(); it != timestamps.end(); ++it) {
            ...
            if (oldChord != maxChord) {
                ...
                chord_feature.label = m_chordnames[maxChordIndex];
                 cerr << "CHORD -------------------- " <<  m_chordnames[maxChordIndex] << "  "; 
   ...
   // for (int iFrame = 0; iFrame < nFrame; iFrame++) cerr << "PRinting values - " << m_outputHarmonicChange << endl;
    //fsOut[m_outputHarmonicChange][iFrame].values[0] << endl;
